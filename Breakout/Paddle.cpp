...
void Paddle::moveLeft(float dt)
{
    ...
    if ((sf::Keyboard::isKeyPressed(sf::Keyboard::A) || sf::Mouse::isButtonPressed(sf::Mouse::Left)) && position > 0)
    {
...
void Paddle::moveRight(float dt)
{
    ...
    if ((sf::Keyboard::isKeyPressed(sf::Keyboard::D) || sf::Mouse::isButtonPressed(sf::Mouse::Left)) && position < _window->getSize().x - _width)
    {
        _sprite.move(sf::Vector2f(dt * PADDLE_SPEED, 0));
    }
}
